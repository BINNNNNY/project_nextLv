...
<form method="post" action="write_ok.php">
  ...
  <div class="mb-3">
    ...
    <select name="sub_region" id="sub_region" class="form-select mt-2" required>
  ...
  <div class="mb-3">
    ...
    <select name="fraud_type" class="form-select">
      ...
      <?php foreach ($fraud_types as $type): ?>
        <option value="<?= $type ?>" <?= $fraud_type === $type ? 'selected' : '' ?>><?= $type ?></option>
      <?php endforeach; ?>
    </select>
  </div>
